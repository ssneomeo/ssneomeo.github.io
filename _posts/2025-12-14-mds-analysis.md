...
기존의 주가 데이터 분석에서 나는 보통 시계열 분석기법을 활용해 주가의 시간적 변동에 초점을 맞췄는데, 여기서는 종목간 주가 변동의 관계(상관계수)를 공간상의 거리로 간주하고, 각 종목을 공간상에 표현해봤음.

 더 구체적으로는 sp500 주식 종목들에 대한 분당 종가 데이터를 바탕으로 **피어슨 상관계수(Pearson Correlation)**를 거리 함수로 정의하고, **MDS(다차원 척도법)**을 이용해 이를 시각화했음. <- 이를 통해 시장의 위상구조와 섹터별 군집화 현상을 관찰해봤음.
