...
categories: [Quant]           # [대분류, 소분류]
tags: [python, data-analysis]         # 태그 (소문자 권장)
...
주식시장은 수천 개의 종목이 서로 상호작용하며 상태가 바뀌는 **거대한 복잡계(Complex System)**임. 근데 열역학적 운동과는 다르게 종목들은 랜덤하게 움직이지 않고 각자 특정 경향성을 가진 채로 움직임. <- 이 경향성을 잘 파악할 수 있다면 주식시장 시스템에 대한 이해에 더해 알고리즘 매매 전략 수립에서 유용한 피쳐 데이터로 사용할 수도 있을 것 같음.

기존의 주가 데이터 분석에서 나는 보통 시계열 분석기법을 활용해 주가의 시간적 변동에 초점을 맞췄는데, 여기서는 종목간 주가 변동의 관계(상관계수)를 공간상의 거리로 간주하고, 각 종목을 공간상에 표현해보았음.

 더 구체적으로는 sp500 주식 종목들에 대한 분당 종가 데이터를 바탕으로 **피어슨 상관계수(Pearson Correlation)**를 거리 함수로 정의하고, **MDS(다차원 척도법)**을 이용해 이를 시각화했음. <- 이를 통해 시장의 위상학적 구조와 섹터별 군집화 현상을 기하학적으로 관찰해 보았음.

## 2. 배경

### 2.1. 상관관계의 거리 변환 (Metric Definition)
주가 $i$와 $j$의 상관계수 $\rho_{ij}$는 $-1$에서 $1$ 사이의 값을 가짐. 이를 유클리드 공간상의 거리(Metric)로 변환하기 위해 다음과 같은 수식을 사용하는데, 참고로 이건 정규화된 수익률 벡터 사이의 유클리드 거리와 수학적으로 동치임.
